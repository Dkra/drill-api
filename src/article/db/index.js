...
const read = (id) => new Promise((res, rej) => {
  const article = articleDB.article.filter(a => a._id === +id)
  console.log('articleDB.article', articleDB.article);
...
const remove = (id) => new Promise((res, rej) => {
  try {
    ...
    if (originLength === articleDB.article.length) throw new Error(`Article Id:${id} not found!`)
